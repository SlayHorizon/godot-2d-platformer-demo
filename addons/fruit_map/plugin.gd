...
const COLLECTIBLE = preload("res://source/maps/objects/collectible/collectible.tscn")
const GRID_STEP := Vector2(16, 16)
...
func add_interface() -> void:
	...
	fruit_selection_interface.custom_minimum_size = Vector2(100, 100)
	...
	for fruit: int in Fruit.Fruits.values():
		var fruit_texutre := Fruit.get_fruit_texture(Fruit.Fruits.keys()[fruit].capitalize())
		var fruit_preview := get_fruit_texture_preview(fruit_texutre)
		add_fruit_button(fruit_preview, fruit)
...
func add_fruit_action() -> void:
	...
	var undo_redo_manager := get_undo_redo()
...
func remove_fruit_action() -> void:
	var viewport_2d: SubViewport = EditorInterface.get_editor_viewport_2d()
	var direct_space_state := viewport_2d.get_world_2d().direct_space_state
	var parameters := PhysicsPointQueryParameters2D.new()
	parameters.position = selected_fruit_map.get_global_mouse_position()
	parameters.collide_with_areas = true
	var result := direct_space_state.intersect_point(parameters)
	if not (
		result.size()
		and result[0] is Dictionary
		and (result[0] as Dictionary).has("collider")
	):
		return
	# Will cast to null if the target isn't a Fruit
	var fruit: Fruit = result[0]["collider"] as Fruit
	if fruit is Fruit and fruit.get_parent() == selected_fruit_map:
		var undo_redo_manager := get_undo_redo()
